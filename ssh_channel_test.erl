...
create_netconf_channel(Connection) ->
    case ssh_connection:session_channel(Connection, infinity) of
        {ok, Channel} ->
            ...
            io:format(
                "Attempting to start NETCONF subsystem on Channel(~p)~n", [
                    Channel
                ]
            ),
            case
                ssh_connection:subsystem(
                    Connection, Channel, "netconf", infinity
                )
            of
                success ->
                    io:format("NETCONF subsystem started on Channel(~p)~n", [
                        Channel
                    ]),
                    {ok, Channel};
                Else ->
                    io:format(
                        "Failed to start NETCONF subsystem on Channel(~p): ~p~n",
                        [Channel, Else]
                    ),
